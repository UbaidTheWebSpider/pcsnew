...
const ProfileContent = ({ patientId }) => {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center h-96 z-10 bg-white rounded-3xl">
            ...
            <p className="text-gray-500 font-semibold">Assembling Profile...</p>
        </div>
    );
    ...
    if (error) return (
        <div className="flex flex-col items-center justify-center h-96 z-10 bg-white rounded-3xl">
            ...
            <p className="text-gray-900 font-bold">Connection Error</p>
                ...
                <div className="relative flex items-center gap-8 z-10">
                    ...
                    <div className="flex-1 text-white">
                        <div className="flex items-center gap-4 mb-3">
                            <h2 className="text-4xl font-bold tracking-tight leading-none text-transparent bg-clip-text bg-gradient-to-r from-white via-white to-slate-400">{patient.name}</h2>
                            <span className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-[0.2em] border border-slate-700 bg-slate-800/50 backdrop-blur-md text-indigo-400`}>
                                {patient.patientType || 'OPD'}
                            </span>
                        </div>
                        <div className="flex flex-wrap items-center gap-6 text-slate-400 text-[11px] font-semibold uppercase tracking-widest">
                            <span className="flex items-center gap-2"><Shield size={14} className="text-indigo-400" /> CNIC: <span className="text-slate-100 font-mono tracking-tighter font-bold">{patient.cnic || 'Unverified'}</span></span>
                            <span className="flex items-center gap-2"><Activity size={14} className="text-rose-400" /> Blood: <span className="text-slate-100 font-bold">{patient.bloodGroup || 'O+'}</span></span>
                            <span className="px-3 py-1 bg-slate-800/80 rounded-full border border-slate-700 flex items-center gap-2">
                                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                                ID: <span className="text-slate-100 font-mono font-bold">{patient.patientId}</span>
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div className="px-10 border-b border-gray-200 bg-white flex items-center gap-10 overflow-x-auto no-scrollbar">
                {[
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2.5 py-5 text-[11px] font-bold uppercase tracking-widest transition-all relative ${activeTab === tab.id
                            ? 'text-indigo-600'
                ...
                {activeTab === 'overview' && (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            ...
                            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-y-10 gap-x-6">
                                    <div>
                                        ...
                                        <p className="text-gray-900 font-bold text-sm">{patient.name}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1.5">Father's Name</p>
                                        <p className="text-gray-900 font-bold text-sm">{patient.fatherName || 'Not Provided'}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1.5">Biological Sex</p>
                                        <p className="text-gray-900 font-bold text-sm uppercase">{patient.gender || 'Unknown'}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1.5">Date of Birth</p>
                                        <p className="text-gray-900 font-bold text-sm">{patient.dateOfBirth ? new Date(patient.dateOfBirth).toLocaleDateString(undefined, { dateStyle: 'medium' }) : 'N/A'}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1.5">Calculated Age</p>
                                        <p className="text-gray-900 font-bold text-sm">{patient.age || '?'} Years</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1.5">Blood Group</p>
                                        <p className="text-rose-600 font-bold text-sm">{patient.bloodGroup || 'Not Tested'}</p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-6">
                                        <div className="flex items-center gap-4 group">
                                            ...
                                            <div>
                                                <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Primary Phone</p>
                                                <p className="text-gray-800 font-bold text-sm">{patient.contact?.phone || patient.phone || 'N/A'}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-4 group">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-gray-800 font-bold text-sm">{patient.email || patient.contact?.email || 'No email registered'}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="p-5 bg-slate-50/50 rounded-2xl border border-slate-100">
                                        <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-2 flex items-center gap-1.5"><MapPin size={10} /> Street Address</p>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 p-8 rounded-3xl shadow-xl text-white relative overflow-hidden">
                                ...
                                {patient.emergencyContact?.name ? (
                                    <div className="space-y-4 relative z-10">
                                        <div>
                                            <p className="text-indigo-300 text-[10px] font-bold uppercase mb-1">Contact Person</p>
                                            <p className="text-xl font-bold leading-none">{patient.emergencyContact.name}</p>
                                            <p className="text-indigo-200 text-xs font-semibold mt-1 opacity-80">{patient.emergencyContact.relation || 'Relation not specified'}</p>
                                        </div>
                                        <div className="pt-4 border-t border-white/10">
                                            <p className="text-indigo-300 text-[10px] font-bold uppercase mb-1">Direct Line</p>
                                            <p className="text-lg font-mono font-bold">{patient.emergencyContact.phone}</p>
                                        </div>
                                    </div>
                                ) : (
                            ...
                            {/* Status Card */}
                            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                                <h4 className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-6">Patient Lifecycle</h4>
                                <div className="space-y-6">
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-semibold text-gray-600">Current Status</span>
                                        <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-tighter ${patient.status === 'Active' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}>
                                            {patient.status || 'Active'}
                                        </span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-semibold text-gray-600">Registration</span>
                                        <span className="text-xs font-bold text-gray-900">{new Date(patient.createdAt).toLocaleDateString()}</span>
                                    </div>
                        ...
                        <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl border border-indigo-50 max-w-2xl w-full flex flex-col items-center">
                            {patient.healthId ? (
                                <>
                                    ...
                                    <div className="bg-indigo-50 border border-indigo-100 text-indigo-700 px-6 py-3 rounded-2xl text-[11px] font-bold uppercase tracking-widest flex items-center gap-3">
                                        <CheckCircle size={14} /> Registered Health ID: <span className="text-sm font-mono tracking-normal font-bold">{patient.healthId}</span>
                                    </div>
                                </>
                            ) : (
                                <div className="text-center py-12">
                                    ...
                                    <h3 className="text-3xl font-bold text-slate-900 mb-3 tracking-tight">Issue Health ID</h3>
                                    <p className="text-slate-500 mb-10 max-w-sm mx-auto font-semibold text-sm leading-relaxed">Activate cryptographic identity for instant hospital admission and digital record syncing across the network.</p>
                                    <button
                                        onClick={handleGenerateHealthId}
                                        className="bg-indigo-600 text-white px-10 py-4 rounded-2xl hover:shadow-2xl hover:shadow-indigo-500/40 transition-all font-bold uppercase tracking-widest text-[11px] flex items-center gap-3 mx-auto active:scale-95"
                                    >
                ...
                {activeTab === 'entitlements' && (
                    <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-gray-100 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="flex items-center justify-between mb-10 pb-6 border-b border-indigo-50">
                            <div>
                                <h3 className="text-2xl font-semibold text-slate-900 tracking-tight">Coverage Intelligence</h3>
                                <p className="text-slate-400 text-xs font-semibold uppercase tracking-widest mt-1">Real-time insurance & benefit verification</p>
                            </div>
                            <span className="bg-emerald-50 text-emerald-700 px-6 py-2 rounded-xl font-semibold uppercase tracking-[0.2em] text-[10px] border border-emerald-100 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 hover:border-indigo-200 transition-colors group">
                                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-2 group-hover:text-indigo-600 transition-colors">Service Tier</p>
                                <p className="text-xl font-bold text-slate-900">{patient.entitlementDetails?.coverage || 'Full Coverage'}</p>
                            </div>
                            <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 hover:border-indigo-200 transition-colors group">
                                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-2 group-hover:text-indigo-600 transition-colors">Linked Dependents</p>
                                <p className="text-xl font-bold text-slate-900">{patient.entitlementDetails?.dependents?.length || 0} Records</p>
                            </div>
                        </div>

                        <div className="mt-10 p-6 bg-indigo-50/30 rounded-2xl border border-indigo-50">
                            <p className="text-indigo-600 text-[10px] font-bold uppercase tracking-widest mb-2">Coverage Notes</p>
                            <p className="text-slate-600 text-sm font-semibold italic">Patient is entitled to standard OPD and IPD services with no prior authorization required for primary care visits.</p>
                        </div>
                    </div>
                )}
            </div>
